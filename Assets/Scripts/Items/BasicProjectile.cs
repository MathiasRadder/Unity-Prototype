...
public class BasicProjectile : MonoBehaviour
{
    ...
    [SerializeField]
    private bool _targetTime = false;

    [SerializeField]
    private bool _nockBack = false;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (_targetTime)
        {
            BasicTimeCountdown otherTimeCD = other.GetComponent<BasicTimeCountdown>();
            if (otherTimeCD != null)
            {
                otherTimeCD.TimeTrade(-_damage);

                if (_nockBack)
                {

                     other.transform.position += transform.forward *0.8f;
                }
                Kill();
            }
        }
